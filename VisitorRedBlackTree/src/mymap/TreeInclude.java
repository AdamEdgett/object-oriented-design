...
public class TreeInclude<K, V> extends MyTreeMap<K, V> {
    ...
    public MyMap<K, V> accept(MyMapVisitor mapVisitor) {
        ...
        MyTreeMap<K, V> visitedRight =
                (MyTreeMap) this.right.accept(mapVisitor);
        V visitedValue = (V) mapVisitor.visit(this.key, this.value);
        return new TreeInclude<K, V>(visitedLeft, visitedRight, this.key,
